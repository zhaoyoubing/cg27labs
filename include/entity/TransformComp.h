...
// Represents hierarchy links using ECS Entity IDs (or uint32_t)
struct ParentComp {
    uint32_t parentEntityID;
};

struct ChildComp {
    std::vector<uint32_t> childrenEntityIDs;
};

// Optional: The cache component to store the final computed world matrix
// Similar to TransformGPUComp
struct GlobalTransformComp {
